...
# ── ggbrace compatibility shim ──────────────────────────────────────────────
# The original paper used ggbrace 0.1.0 which exported geom_brace().
# Current versions (>=0.1.2) only export stat_brace()/stat_bracetext().
# This shim restores the geom_brace() interface used throughout the script.
if (!exists("geom_brace", mode = "function", envir = asNamespace("ggbrace"))) {
  geom_brace <- function(mapping = NULL, data = NULL, inherit.data = TRUE, ...) {
    # Extract parameters from ...
    dots <- list(...)
    rotate    <- dots$rotate %||% 0
    label_txt <- NULL
    label_size <- dots$labelsize %||% 3
    label_dist <- dots$labeldistance %||% 0.05
    color     <- dots$color %||% "black"
    lineheight <- dots$lineheight %||% 1

    # Evaluate the mapping to get x, y, and optionally label
    map_quos <- mapping
    env <- parent.frame()

    if (!is.null(map_quos$label)) {
      label_txt <- rlang::eval_tidy(map_quos$label, data = data, env = env)
      if (length(label_txt) > 0) label_txt <- label_txt[1]
    }

    x_vals <- rlang::eval_tidy(map_quos$x, data = data, env = env)
    y_vals <- rlang::eval_tidy(map_quos$y, data = data, env = env)

    # Build a 2-row data frame for stat_brace
    brace_df <- data.frame(x = as.numeric(x_vals[1:2]), y = as.numeric(y_vals[1:2]))

    # Merge in faceting columns from data (if provided and inherit.data)
    if (!is.null(data) && inherit.data) {
      facet_cols <- setdiff(names(data), c("x", "y", "mean"))
      for (col in facet_cols) {
        brace_df[[col]] <- data[[col]][1]
      }
    }

    layers <- list(
      stat_brace(
        data = brace_df, mapping = aes(x = x, y = y),
        rotate = rotate, inherit.aes = FALSE, color = color
      )
    )

    if (!is.null(label_txt)) {
      layers <- c(layers, list(
        stat_bracetext(
          data = brace_df, mapping = aes(x = x, y = y),
          rotate = rotate, inherit.aes = FALSE, color = color,
          label = label_txt, size = label_size,
          textdistance = label_dist, lineheight = lineheight
        )
      ))
    }
