...
## Numbers that are not contained in tables or figures
...
### Outcome models with all respondents
...
### Outcome models with inaccurate subgroup
...
### Accuracy model with all respondents
...
```{r fig.width=13.5, fig.height=14.4}
# save figure 1
...
## Mediation Table for the models for Democrats only

```{r main_mediation_table}
...
### Aligned versus Misaligned Headlines

```{r balance_table_aligned}
...
```{r balance_table_accuracy_order}
generate_balance_table(data = df.experiment_long_balance_headline_1, 
                       covariates = vec.controls_balance, 
...
### Partisanship in the Inaccurate Subset

```{r balance_table_prep_2}
# create dataframe for balance tables with capitalized variable names
...
### Descriptive Statics of Final Sample

```{r descriptive_stats_data_prep}
...
```{r representativeness_ipsos_full_sample, eval = FALSE}
...
```{r representativeness_ipsos_nonstudent_sample, eval = FALSE}
...
```{r representativeness_ipsos_subsample, eval = FALSE}
...
```{r representativeness_final_sample, eval = FALSE}
...
```{r representativeness_table, eval = FALSE}
...
### Main models (main specification and robustness checks in separate plots)
...
```{r fig.height=17.1, fig.width=18}
...
### Robustness check for main models with pre-registered censorship coding
...
### Models With Selected Headlines
...
## Consensus Headline Overview Table
...
### Main models shown as plots in main text

```{r regression_tables_main}
...
### Models Disaggregated by Headline
...
### Models With Triple Interaction
...
### Models for Democrats only

#### 4-point accuracy measure
...
## Mediation Sensitivity Analyses
...
## Frequency of Censorship-Related Keywords in Congressional Speeches
...
Data source for majority party information: History, Art & Archives, U.S. House of Representatives, Party Government Since 1857 [Dataset]. https://history.house.gov/Institution/Presidents-Coinciding/Party-Government/ (2022).
