...
graduation$nsibs_factor <- factor(graduation$nsibs)

m_full_nsib_factor <-glm (
  hsgrad ~ nonwhite + mhs + fhs + income + asvab + nsibs_factor + intact,
  binomial(link = logit),
  data = graduation
)

summary(m_full_nsib_factor)

anova(m_full, m_full_nsib_factor, test = "LRT")
...
anova(m_full_clean, m_full_nsibs_cat, test = "LRT")